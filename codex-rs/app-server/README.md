...
If the session approval policy uses `Reject` with `request_permissions: true`, inline `with_additional_permissions` command requests are auto-denied. The standalone `request_permissions` tool still sends `item/permissions/requestApproval`, and any granted permissions remain sticky for later shell-like calls in the same turn.
